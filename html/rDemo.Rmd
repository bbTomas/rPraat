...
```{r, fig.height=3}
snd1 <- list(fs = 8000, sig = sin(seq(0, 2*pi, length.out = 4000)))
snd.plot(snd1)
snd2 <- list(t = seq(0, 3999)/8000, sig = sin(seq(0, 2*pi, length.out = 4000)))
snd.plot(snd2)

fs <- 48000
t <- seq(0, fs-1)/fs  # duration of 1 second
sig <- 1*sin(2*pi*1000*t)
snd.write(list(sig = sig, nBits = 16, fs = fs), "demo/temp.wav")
