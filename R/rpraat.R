...
#' @param from Where to start reading in \code{units} (beginning \code{"sample"}: 1, \code{"seconds"}: 0)
#' @param to Where to stop reading in \code{units} (\code{Inf} = end of the file)
#' @param units Units of \code{from} and \code{to} argument: \code{"samples"} (starting from 1) or \code{"seconds"} (starting from 0)
#'
...
snd.read <- function(fileNameSound, fileType = "auto", from = 1, to = Inf, units = "samples") {
    ...
    s <- list(t = seqM(0, nSamples-1)/snd@samp.rate,
              sig = sig,
              fs = snd@samp.rate,
